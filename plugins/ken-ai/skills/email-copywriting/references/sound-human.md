...
## The one-on-one favor mindset (the frame everything else hangs on)

AI personalization renders every email as a genuinely individual message - this person's name, their company, their situation. Write from that reality: you are ONE human typing a short note to ONE other human, because you found something that helps them specifically.

The posture is **doing them a favor, not asking for one.** You're usually handing them something - a free deliverable, a fix for a problem they visibly have, information they'd want anyway. Write like the person who shows up with the thing, not the person who wants a slice of their time.

A marketing email talks about the sender: features, "we do X", what the platform offers. A one-on-one note talks about the reader: their situation, the thing they're getting, what happens next. Tests while drafting:

- **The list test:** could this exact email go to 500 people unchanged and read the same? Then it's a marketing email, not a note. The static body IS shared - but it must read like it was typed for this one reader.
- **The favor test:** delete every sentence about the sender's product and company. Is there still something in the email for the reader? "We offer X, our platform does Y, we help teams Z" fails - that's a brochure.
- **Features are the enemy of the note.** A human doing a favor says "I'll build the list and write the emails - you just approve them." A marketer says "our platform offers AI-powered list building and copy generation." Say what THEY get in plain verbs; the mechanism gets one clause at most.

## Deliberate imperfection (small doses)

Real one-on-one emails are typed fast and imperfect. Flawless copy is itself an AI tell (see "Flawless-but-generic" below). It is allowed - and mildly encouraged - to leave 1-2 small human imperfections across a sequence.

Good imperfections (pick from these):
- a lowercase sentence start, or an all-lowercase subject line
- a missing comma where a fast typist would drop one
- casual shorthand: "btw", "tbh", "prob", "thats" without the apostrophe
- a slightly run-on sentence joined with a dash or "and"
- a tacked-on afterthought ("also - we cover the cost")

Hard limits:
- **Max ~1 per email, ~2 per sequence.** Three typos reads sloppy, not human.
- **NEVER in:** the prospect's name, their company, the client's company, links, numbers, prices, or any offer claim. A typo'd name kills the email.
- **No scrambled-letter fake typos** ("teh", "recieve") - they distract. Dropped punctuation and casual shorthand beat misspelled words.
- Don't manufacture imperfections mechanically. Write fast and casual, then keep the natural roughness instead of polishing it out.
...
Rules:
- The card derives from `context.md`, `client_research.md`, and the segment definition - not from imagination. If the client has a founder voice profile in notes/memory, it overrides the generic sender persona.
- Copy the finished card into the `## Notes` section at the end of `emails.md` so email-review can check the copy against it.
