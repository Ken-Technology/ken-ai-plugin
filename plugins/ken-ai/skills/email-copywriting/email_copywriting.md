...
*   **Write the double-brace form directly** - `{{First Line}}` is exactly what Ken AI stores and renders, so author it that way for correct rendering on push. (The legacy `[First Line]` square-bracket form still auto-converts for old files, but use `{{...}}` in all new copy.)
