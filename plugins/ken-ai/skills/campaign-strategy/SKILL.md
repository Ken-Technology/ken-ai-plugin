...
- Never reference data sources the parser and backend don't support (news, funding, events, social media, blogs) - these propagate into prompts.md and the backend will error.
...
**Directory Structure Reference** (`{workspace}` = the client campaign workspace, default `./ken-campaigns/{client-slug}/`):
```
{workspace}/
