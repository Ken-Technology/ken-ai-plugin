...
This document specifies the **text-review half** of the unified Review Phase (Task 3(a) in SKILL.md). The Review Phase as a whole (text review + live campaign test) auto-skips when `{plan_folder}/configuration.json` has no `campaign_id` - so the text review described here also skips when there's no campaign to test against.

No user input required when it runs.

## Overview

The text review ensures high-quality AI personalization prompts by running an internal reviewer subagent over the draft before the live campaign test (`api_campaign_prompt_test`) runs. Both halves happen silently - do NOT output intermediate steps to the user.

## Reviewer Agent Spec

The spec has three conceptual stages: the draft (already produced by Task 2 in SKILL.md), the reviewer pass (below), and the writer's revision pass (apply feedback, still within Task 3(a) in SKILL.md).
...
- User prompts (to_output: true) - these are the only prompts CSMs write
- Data source validity
- Prompt quality (constraints, fallbacks, tone)
- Variable coverage from strategy
- Rewriting Instructions steering paragraph (if present): length 2-10 sentences, does not duplicate backend default rules, focuses on voice/tone/sequence flow, no examples or formatting rules

**DO NOT review:**
- System prompts (auto-generated by backend: Instructions, Rules, Company Context, Email Sequence)
- The default rewriting prompt (auto-injected by backend - only the user's steering paragraph is in scope)
...
This reviewer pass is invoked inside **Task 3(a) (Internal Text Review)** of the unified Review Phase in SKILL.md. The Review Phase gate - a non-null `configuration.json.campaign_id` - applies to this pass as well: if the gate fails, the reviewer subagent is NOT spawned and `prompts.md` is written directly.
