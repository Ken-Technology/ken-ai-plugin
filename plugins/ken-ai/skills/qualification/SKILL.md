...
Write the structured variables for AI prospect qualification. Output maps directly to Ken AI PromptLibrary structured fields (Flow=Qualification).
...
2. **Read `filters.json`** from the plan folder - Search filters already applied. Never restate any of these dimensions in the prompt.
3. **Read `{workspace}/research.md`** - Client overview, detailed ICP info, competitors (`{workspace}` = the client campaign workspace, default `./ken-campaigns/{client-slug}/` under the current directory)
...
- **Never repeat the search filters** - filters.json already bounds the list (headcount, geography, titles, seniority, industries). Do NOT restate any of it - not in the audience description, not as a criterion. If the filter sets a headcount range, the prompt says nothing about headcount. Qualification only catches what the filters can't see.
...
- [ ] Read filters.json for what's already filtered
...
- `audience_description` -> PromptLibrary field `audience_description`
- `disqualification_criteria` -> PromptLibrary field `disqualification_criteria` (bullet points joined as text)
- `qualification_criteria` -> PromptLibrary field `qualification_criteria` (bullet points joined as text)
