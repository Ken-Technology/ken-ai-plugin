...
End-to-end cold-email campaign creation on the [Ken AI platform](https://ken.so), packaged as a plugin for **Claude Code and OpenAI Codex CLI**. Plan a campaign, build the prospect targeting, write and review the copy, generate AI personalization prompts, and push everything to Ken AI - all through the ken-ai MCP server's OAuth session. No API keys to paste, no local scripts, no `.env`.
